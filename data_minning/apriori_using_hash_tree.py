...
class HashTreeNode:
    def __init__(self, depth=0, max_leaf_size=10):
        self.children = {}
        self.itemsets = []
        self.is_leaf = True
        self.depth = depth
    ...
    def insert(self, itemset):
        if self.is_leaf:
            ...
            if len(self.itemsets) > self.max_leaf_size and self.depth < len(next(iter(self.itemsets))):
                self.split()
        else:
            items = sorted(list(itemset))
            if self.depth < len(items):
                idx = items[self.depth]
                if idx not in self.children:
                    self.children[idx] = HashTreeNode(self.depth + 1, self.max_leaf_size)
                self.children[idx].insert(itemset)
    ...
    def query(self, transaction, found=None):
        if found is None:
            found = []
        if self.is_leaf:
            for itemset in self.itemsets:
            ...
            for idx in set(items[self.depth:]):
                if idx in self.children:
                    self.children[idx].query(transaction, found)
        return found
...
def get_frequent_itemsets(transactions, min_sup):
    ...
    while Lk:
        ...
        for transaction in transactions:
            ...
            # Lấy các candidate là tập con của transaction bằng hash tree
            found_candidates = hash_tree.query(t_set)
